...
router.get("/", allowToAdminOnly, async (req, res, next) => {
  try {
    let list = await RateService.getAllRates(req);
...
router.get("/:id", allowToAdminOnly, async (req, res, next) => {
  try {
    ...
    let obj = await RateService.getRateById(req, id);
    res.send(obj);
  } catch (error) {
    next(error);
  }
...
router.post("/", allowToAdminOnly, upload.any(), async (req, res, next) => {
  try {
    ...
    // Check if a rates record already exists for this tenant
    const existingRates = await RateService.getAllRates(req);
    if (existingRates.length > 0) {
      // Update existing record instead of creating a new one
      obj._id = existingRates[0]._id;
      obj.updateDate = new Date();
      if (req.tokenData && req.tokenData.name) {
        obj.updatedBy = req.tokenData.name;
      }
      let result = await RateService.updateRate(req, obj);
      if (result) {
        return res.status(200).json(result);
      } else {
        return res.status(404).json({ error: "Rate not found or unauthorized" });
      }
    }
    ...
    obj = await RateService.addRate(req, obj);
...
router.put("/", allowToAdminOnly, upload.any(), async (req, res, next) => {
  try {
    ...
    if (result) {
      ...
      res.status(404).json({ error: "Rate not found or unauthorized" });
    }
  } catch (error) {
    next(error);
  }
...
router.delete("/:id", allowToAdminOnly, async (req, res, next) => {
  try {
    let id = req.params.id;
    let obj = await RateService.deleteRate(req, id);
    res.json(obj);
  } catch (error) {
    next(error);
  }
