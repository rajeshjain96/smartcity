const express = require("express");
const router = express.Router();
const ShopDetailService = require("../services/shopDetail.service");
const multer = require("multer");
const path = require("path");
const { normalizeNewlines } = require("../utilities/lib");

const storage = multer.diskStorage({
  destination: function (req, file, cb) {
    // Use absolute path - uploads folder is sibling of server folder
    const uploadsPath = path.join(__dirname, "..", "..", "uploads");
    cb(null, uploadsPath);
  },
  filename: function (req, file, cb) {
    cb(null, file.originalname);
  },
});
const upload = multer({ storage: storage });

router.get("/", allowToAdminOnly, async (req, res, next) => {
  try {
    let list = await ShopDetailService.getAllShopDetails(req);
    res.status(200).json(list);
  } catch (error) {
    next(error);
  }
});

router.get("/:id", allowToAdminOnly, async (req, res, next) => {
  try {
    let id = req.params.id;
    let obj = await ShopDetailService.getShopDetailById(req, id);
    res.send(obj);
  } catch (error) {
    next(error);
  }
});

router.post("/", allowToAdminOnly, upload.any(), async (req, res, next) => {
  try {
    let obj = req.body;
    const keys = Object.keys(obj);
    for (let key of keys) {
      if (typeof obj[key] == "string") {
        obj[key] = normalizeNewlines(obj[key]);
      }
    }
    // Handle file uploads - files are in req.files array
    if (req.files && req.files.length > 0) {
      console.log("Files received:", req.files.map(f => ({ fieldname: f.fieldname, filename: f.filename })));
      req.files.forEach((file) => {
        // The fieldname from the form (e.g., "file0") determines which field this is
        // For shopDetails, we expect "file0" for logo
        if (file.fieldname === "file0" || file.fieldname.startsWith("file")) {
          obj.logo = file.filename;
        }
      });
    } else {
      console.log("No files received in req.files");
    }
    obj.addDate = new Date();
    obj.updateDate = new Date();
    obj = await ShopDetailService.addShopDetail(req, obj);
    res.status(201).json(obj);
  } catch (error) {
    next(error);
  }
});

router.put("/", allowToAdminOnly, upload.any(), async (req, res, next) => {
  try {
    let obj = req.body;
    // Handle file uploads - files are in req.files array
    if (req.files && req.files.length > 0) {
      console.log("Files received:", req.files.map(f => ({ fieldname: f.fieldname, filename: f.filename })));
      req.files.forEach((file) => {
        // The fieldname from the form (e.g., "file0") determines which field this is
        // For shopDetails, we expect "file0" for logo
        if (file.fieldname === "file0" || file.fieldname.startsWith("file")) {
          obj.logo = file.filename;
        }
      });
    } else {
      console.log("No files received in req.files");
    }
    obj.updateDate = new Date();
    // Add updatedBy field with user's name if available
    if (req.tokenData && req.tokenData.name) {
      obj.updatedBy = req.tokenData.name;
    }
    let id = obj._id;
    let result = await ShopDetailService.updateShopDetail(req, obj);
    if (result) {
      result._id = id;
      res.status(200).json(result);
    } else {
      res.status(404).json({ error: "Shop detail not found or unauthorized" });
    }
  } catch (error) {
    next(error);
  }
});

router.delete("/:id", allowToAdminOnly, async (req, res, next) => {
  try {
    let id = req.params.id;
    let obj = await ShopDetailService.deleteShopDetail(req, id);
    res.json(obj);
  } catch (error) {
    next(error);
  }
});

function allowToAdminOnly(req, res, next) {
  if (
    !req.tokenData ||
    req.tokenData.role == "guest" ||
    req.tokenData.role == "user"
  ) {
    res.status(401).json({ message: "Unauthorized" });
  } else if (req.tokenData.role == "admin") {
    next();
  } else {
    res.status(401).json({ message: "OOPs...Some Error.." });
  }
}

module.exports = router;

